...
{
	if (!init()) {
		errorMessage = QString("Could not initialize FIDOKey.");
		return false;
	}
    if(slot!=0)
...
FIDOKey::ChallengeResult FIDOKey::challenge(int slot, bool mayBlock, const QByteArray& unpaddedChallenge, QByteArray& response)
{
	// ensure that FIDOKey::init() succeeded
	if (!init()) {
		return ERROR;
	}
